...
構築したLangGraphベースのエージェント：
- **エンドポイント**: `{agent_endpoint}`（Model Serving）
...
```sql
CREATE OR REPLACE FUNCTION yao_sl_st_catalog.{schema}.ask_agent(
...
Managed MCPで公開：
```
https://<workspace>/api/2.0/mcp/functions/yao_sl_st_catalog/{schema}
```
...
| **Code Agent** | `POST /serving-endpoints/{name}/invocations` | ResponsesAgent (`input`/`output`) | Bearer PAT | ○ 同期だが形式変換要 |
